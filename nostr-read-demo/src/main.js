...
async function processHistoricMessages(kv, relay) {
  ...
  if (delta < 0) {
    throw new Error(
      `Time went backwards: last ${last_processed} -> now ${now}`,
    );
  }
  ...
  const trunks = new Array(trunkCount).fill(null).reduce((prev, _, i, arr) => {
  ...
  for (const t of trunks) {
    console.log(
      `(${1 + t.i}/${trunks.length}) Processing messages from ${t.from} to ${t.to}`,
    );
